...
import { Search, X, Maximize2, Loader2, AlertCircle, FileCode } from "lucide-react";
  ...
  if (graph.order > 0) {
    ...
    if (minX !== Infinity) {
      ...
      graph.forEachNode((node) => {
        const x = graph.getNodeAttribute(node, "x") as number;
        const y = graph.getNodeAttribute(node, "y") as number;
        graph.setNodeAttribute(node, "x", x - centerX);
        graph.setNodeAttribute(node, "y", y - centerY);
      });
    }
  }
  return graph;
...
function GraphInteractionLayer({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTimeout(() => {
        if (sigma.getCamera()) {
          sigma.getCamera().animate({ x: 0, y: 0, ratio: 1.4 }, { duration: 0 });
          sigma.refresh();
        }
      }, 100);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (cameraResetCount > lastResetRef.current) {
      ...
      sigma.getCamera().animate({ x: 0, y: 0, ratio: 1.4 }, { duration: 800 });
      return;
    }

    const focus = selectedNode || focusPath || null;
    if (focus && graph.hasNode(focus)) {
      const { x, y } = graph.getNodeAttributes(focus) as any;
      if (typeof x === "number" && typeof y === "number" && isFinite(x) && isFinite(y)) {
        sigma.getCamera().animate({ x, y, ratio: 0.15 }, { duration: 800 });
      }
    }
  }, [sigma, graph, selectedNode, focusPath, cameraResetCount]);
    ...
    sigma.setSetting("nodeReducer", (node, data) => {
      try {
        ...
        const result: any = {
          ...
          // Force labels for the focus node AND all its immediate links
          forceLabel: isCurrentFocus || (activeFocusNode && connected.has(node)),
        };
...
export function GephiSigmaGraph({
  ...
  return (
    <div className={cn("h-full min-h-[500px] flex flex-col bg-white rounded-2xl overflow-hidden border border-slate-200/60 shadow-sm", className)}>
